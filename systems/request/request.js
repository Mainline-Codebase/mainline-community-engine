...
const {
  abi, source, secrets, returnType, gasLimit: reqGasLimit,
} = getStaticParams();

const sendChainlinkRequest = async (args, networkName, updateToast) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let requestId;
      const returned = { result: null, cost: null, error: false };
      ...
      if (updateToast) { updateToast(`Waiting ${VERIFICATION_BLOCK_CONFIRMATIONS} blocks for the request transaction to be confirmed...`); }
...
module.exports = sendChainlinkRequest;
